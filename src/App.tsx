  ...
  */
    ...
    function generateSpiralData(
        ...
        for (let i = 0; i < numSamples / 2; i++) {
            ...
            const t =
                ((1.75 * i) / numSamples) * 2 * Math.PI + Math.random() * noise;
    ...
    const testData = generateSpiralData(200, 0.1);
    ...
    useEffect(() => {
        ...
        trainingData.forEach((data) => {
            ...
            updates.push(
                `Input: [${data.input}] | Target: [${data.target[0]}] | Prediction: [${output[0]}]`
            );
            error += Math.abs(
                data.target.reduce(
                    (acc, v, i) => acc + Math.abs(v - output[i]),
                    0
                )
            );
        });
        ...
        setUpdate(
            [
                ...updates,
                `Error: ${error}`,
                `Weights in ${JSON.stringify(w)}`,
                `Weights out ${JSON.stringify(w1)}`,
            ].join("\n")
        );
